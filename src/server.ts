...
function startVotingPhase() {
  if (game.submissions.length < 2) {
    if (game.hostSocketId) {
      io.to(game.hostSocketId).emit("errorMessage", {
        message: "Need at least 2 submissions to vote.",
      });
    }
    return;
  }
...
io.on("connection", (socket) => {
  ...
  socket.on("submitAnswer", ({ text }: { text?: string }) => {
    ...
    emitGameState();
    if (
      game.submissions.length === game.players.length &&
